...
This project is implemented on stacked feature branches. The current tip for ingestion is `feature/ingest-cli`. `main` still has only the initial policy files.

The source policy lives in `policy.md`. Embeddings use `all-MiniLM-L6-v2` (384 dimensions, L2-normalized). Later retrieval will rank with pgvector cosine distance (`<=>`). Six rows are scanned sequentially; an ANN index would not help at this size.

## Setup

```bash
git checkout feature/ingest-cli
python -m venv .venv
...
Default `pytest` stays offline. Integration tests that download MiniLM are opted in with `pytest -m integration`.
